  ...
  <div class="invoice p-3 mb-3" id="mydiv">
    ...
    <div class="row invoice-info">
      ...
      <div class="col-sm-5 invoice-col"  style="text-align: center;">
        <b>&nbsp;&nbsp;&nbsp;Invoice No:</b>
        <span>{{form.vv_invoice_no}}</span>
        <br>
        <b>Invoice Date:</b>
        <span>{{form.vv_invoice_date | myDate }}</span>
        <br>
      </div>
      <!-- /.col -->
    </div>
    ...
    <div class="row">
      <div class="col-12 table-responsive">
        <table class="table table-striped" style="width:100%">
          ...
          <tbody>
            <tr v-for="(vvInvoiceItem, key) in form.vv_invoice_items" :key="key">
              <td>{{vvInvoiceItem.vv_passenger_name}}</td>
              <td width="60%" style="text-align: center;">{{vvInvoiceItem.vv_description}}</td>
              <td>{{formatPrice(vvInvoiceItem.vv_fares)}}</td>
              <td>{{formatPrice(vvInvoiceItem.vv_sub_total)}}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="col-8">
        <p class="lead">
          <small class="text-muted well well-sm no-shadow">
            ...
            <span style="text-transform: capitalize;">{{form.vv_invoice_grand_total | toWords}} Rupees Only.</span>
          </small>
        </p>
      </div>
        ...
        <div class="table-responsive">
          <table class="table">
            <tbody>
              <tr>
                <th>Total Fares</th>
                <td>{{formatPrice(form.vv_invoice_fares_total)}}</td>
              </tr>
              <tr>
                <th>Grand Total</th>
                <td>{{formatPrice(form.vv_invoice_grand_total)}}</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <div class="no-print card-footer" data-html2canvas-ignore="true" style="background-color: lightgray;">
      <div class="col-12">
        ...
        <router-link to="/vv-invoices" class="btn btn-success float-left">
...
export default {
  ...
  loadVvInvoices() {
    axios
      .get("/api/vv-invoice")
      .then(({ data }) => (this.vvInvoices = data.data));
  }
  },
  created: function() {
    axios
      .get("/api/vv-invoice/" + this.$route.params.id)
      .then(({ data }) => {
        console.log(data);
